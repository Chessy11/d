...
class WXRImporter extends \WP_Importer {
	...
	public $stock_images = array();
	public $temp_images = array(
        'post' => 'blog.jpg',
        'brands' => 'brand.jpg',
	...
	private $_img_id_pattern_for_shortcode = '#(( image_1)|( image_2)|( image_3)|( image_4)|( promo__image)|( image)|( back_image))="(\d+)"#';
    
    private function _replace_img_id_in_shortcodes($match){
		$targetkey=get_option('image_for_post',0);
		if((int)$targetkey > 2){
			$targetkey=0;
			update_option('image_for_post',0);
		}else{
			$targetkey=(int)$targetkey+1;
			update_option('image_for_post',$targetkey);
		}	
		if((int)$targetkey > 2){
			$targetkey=0;
		}
		$bimages = $this->stock_images['attachment'];
		$bimage = $bimages[$targetkey];	
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( preg_match( $this->_img_id_pattern_for_shortcode, $data['post_content'] ) ) {
			$meta[] = array( 'key' => '_wxr_import_has_attachment_refs_vc', 'value' => true );
			$requires_remapping = true;
		}
				...
				if($key == '_product_image_gallery'){
					...
					if(isset($this->stock_images[$post['post_type']])){
						if(is_array($this->stock_images[$post['post_type']])){
							...
							$val=rand(0,2);
							$value = $this->stock_images[$post['post_type']][$val];
						}else{
							$value = $this->stock_images[$post['post_type']];
						}
					}
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			...
			$has_attachments_vc = get_post_meta( $post_id, '_wxr_import_has_attachment_refs_vc', true );
			if ( ! empty( $has_attachments_vc ) ) {
				$post = get_post( $post_id );
				$content = $post->post_content;
				$new_content = preg_replace_callback($this->_img_id_pattern_for_shortcode, array($this, '_replace_img_id_in_shortcodes'), $content );
				$data['post_content']=$new_content;
			}
			...
			delete_post_meta( $post_id, '_wxr_import_has_attachment_refs_vc' );
